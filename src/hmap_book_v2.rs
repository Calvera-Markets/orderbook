...
impl<C: FillConsumer> OrderBook<C> {
    ...
    pub fn cancel_limit_order(&mut self, order_id: OrderId) -> BookResult<()> {
        ...
        // v1 leaked the slot here. With no enum tag the free is just one write
        // to `slots[idx].next` + a `free_head` update.
        self.slab.free(idx);
...
impl HalfBook {
    ...
    /// Cancel-path removal: order is at an arbitrary position in the FIFO,
    /// so we have to stitch both `prev` and `next`. Distinct from the match
    /// path (which only ever consumes from the head) — and from the match
    /// path's bulk free (which donates the existing FIFO chain whole).
