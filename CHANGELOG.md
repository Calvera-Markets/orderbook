...
- **Unified parity suite**: one generic suite in `tests/common`.
- **Databento tape replay** (`examples/tape_replay.rs`): one-shot MBO
  `.dbn.zst` / `--synthetic` harness. Modify is cancel+add.
- **Single engine**: dropped `orderbook_legacy` (v1). The per-side-slab
  matcher is now `src/orderbook.rs`, re-exported at the crate root.
